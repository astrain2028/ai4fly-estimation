...
def one_run(arm, seed, mode, severity):
...
def score(arm, seeds, mode, severity):
    """Average over the seeds. Every arm sees the same ones."""
    rows = [one_run(arm, seed, mode, severity) for seed in seeds]
    return tuple(float(np.mean([r[i] for r in rows])) for i in range(3))


def collect(names, seeds, severities):
    ...
    for label, arm in entries:
        ...
        results[label] = [score(arm, seeds, mode, severity)
                          for _, mode, severity in conditions(severities)]
        print("done")
...
def main():
    ...
    results, missing = collect(names, seeds, severities)
